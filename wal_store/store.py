...
read-only store, every cursor (on either open form) and every resume
session additionally registers a short-lived *lease* -- the one kind of
sidecar a reader is allowed to write, named ``wal.lease.<pid>.<rand>`` --
listing the snapshots that process has in use with a heartbeat. The writer
reads all lease sidecars before reclaiming a copy (re-reading them
immediately before each unlink, so a lease taken mid-sweep still protects
its copy) and keeps every copy a fresh lease names. An orderly close
removes the sidecar; a process killed without closing simply stops
heartbeating, and once its timestamp is older than the TTL -- or its pid
is observed gone, whichever comes first -- its sidecar pins nothing and
is swept. Each lease sidecar is atomically rewritten (temp file, rename)
and only its owner ever writes it, so registering, expiring and
reclaiming -- with a kill at any point, reopened in any process --
converge to the same in-use set and the same copy set. Lease writes are
best-effort, so a genuinely read-only directory simply has no lease.

A published copy is only ever trusted after its content proves its name:
opening one, choosing one for a resume and judging one for reclamation
all re-verify the stored image against the content identity the name
carries. A copy that fails verification is corrupt -- it is never
served, never guessed at and never repaired; the pinned snapshot is
rebuilt from the committed log prefix or the checkpoint instead, and the
corrupt file is removed by the ordinary reclamation sweep. Only when the
...
touch the log, the checkpoint or any snapshot copy: the only file a reader
creates is its own short-lived lease sidecar (best-effort, and absent
altogether in a read-only directory). At open each one picks the highest
committed prefix available at that instant -- the validated ``wal.log``
prefix or ``wal.ckp`` -- scans it once into memory and serves every ``get``
from that index. Because the chosen image is complete and is only ever
replaced atomically (never edited in place), a refresh caught mid-rename, a
half-written commit, a torn record, or a log being rebuilt, shrunk or
compacted can never expose partial bytes: each read lands on one full
committed snapshot that was current at or after the open. A directory
written by an older version is opened the same way from its log and
checkpoint. Reads never replay the log afterwards, so their cost is
independent of history and they never block the writer; a reader that hangs
changes no data, and one that is killed merely leaves a lease that expires
by heartbeat and is swept, never affecting the committed state.
...
class ScanCursor:
    ...
    def __init__(self, items, lo_idx=0, hi_idx=None, pos=None, seq=0,
                 lo=None, hi=None, publisher=None, sid=None, leases=None):
        ...
        # caller usually passes the identity in; otherwise it is derived
        # lazily on the first token mint.
        self._box = _register_snapshot(seq, items, sid)
        # Every cursor -- on a read-only store or on a writer -- holds a
        # cross-process lease for its pinned snapshot (``leases`` is the
        # store's lease registry), so a writer in another process treats
        # it as in use even after the store it came from has closed. The
        # identity is already known by the time the cursor opens.
        self._leases = leases
    ...
    def token(self) -> bytes:
        ...
        of the old log space; a read-only store never publishes a snapshot
        copy when minting (it only holds the cross-process lease registered
        when the cursor opened).
        """
        if self._closed:
            raise ValueError("scan cursor is closed")
    ...
    def close(self) -> None:
        if self._closed:
            return
        _unpin_snapshot(self._box)
        if self._lease_held:
            # Detach the GC finalizer: this explicit release is the one.
            if self._lease_finalizer is not None:
                self._lease_finalizer.detach()
            self._leases.release(self._seq, self._sid)
            self._lease_held = False
        self._items = []
        self._box = None
        self._closed = True
...
# store in *another* process would look free to this writer's reclamation.
# Each lease-holding process therefore owns one short-lived sidecar of the
# single ``wal.lease.*`` kind: it lists the snapshots that process has in use
# together with a heartbeat timestamp, and is atomically rewritten in place.
# The writer reads every lease sidecar before removing a copy and treats
# every fresh one as an in-use pin. An orderly close removes the file; a
# process killed without closing stops heartbeating, so once its timestamp
# is older than the TTL it pins nothing and its stale sidecar is swept.
_LEASE_PREFIX = "wal.lease."
_LEASE_REC_PREFIX = "wal.l"
_LEASE_HEARTBEAT = 0.5
_LEASE_TTL = 5.0
_LEASE_SID_HEX = 64
_LEASE_SEQ_MAX = 20
_LEASE_TS_LEN = 16
# "wal.l<seq20>.<sid64>.<ts16>\n" -- one fixed-width record per pinned
# snapshot. A sidecar is always one or more whole records because it is
# published only by a temp-file/rename, so a reader can never see a torn one.
_LEASE_REC_LEN = (len(_LEASE_REC_PREFIX) + _LEASE_SEQ_MAX + 1
...
class _SnapshotBox:
    """Strong box around one user's materialised, pinned snapshot.

    A box is the unit the reclamation logic counts as a user: every open
    cursor and every read-only store holds its own box for the snapshot it
    serves. Boxes live in a process-wide :class:`weakref.WeakSet`, so a
    ...
    identity is hashed lazily when the caller does not already know it.
    """

    __slots__ = ("seq", "items", "_sid", "__weakref__")

    def __init__(self, seq, items, sid=None):
        self.seq = seq
        self.items = items
        self._sid = sid
...
# every live read-only store keeps a box here for its pinned snapshot; the
# writer's reclamation never removes the published copy of any snapshot
# named in this set. The set is weak, so users closed normally (or simply
# dropped) release their pin with no bookkeeping. The boxes also keep the
# materialised bytes alive in memory, so a token minted in this process
# resolves without touching disk even across stores.
_PINS: "weakref.WeakSet" = weakref.WeakSet()


def _register_snapshot(seq: int, items, sid=None) -> _SnapshotBox:
    """Register one user (a cursor or a reader) pinning a snapshot."""
    box = _SnapshotBox(seq, items, sid)
    _PINS.add(box)
    return box


def _unpin_snapshot(box: _SnapshotBox) -> None:
    """Release the pin held by a closing cursor or reader."""
    _PINS.discard(box)


def _release_lease(leases, seq: int, sid: bytes) -> None:
    """Release a GC'd cursor's cross-process lease; never raises.

    A cursor dropped without ``close()`` releases its in-process pin through
    the weak pin set; this gives the same treatment to its cross-process
    lease so a forgotten cursor cannot pin a copy forever.
    """
    try:
        leases.release(seq, sid)
    except Exception:
        # Finalizers must never raise into the garbage collector.
        pass


def _live_snapshot_keys() -> set:
    """The ``(seq, sid)`` pairs with at least one user in this process."""
    return {(box.seq, box.sid()) for box in _PINS}


def _pinned_snapshot(seq: int, sid: bytes):
    """Materialised items for a live in-process pin, or ``None``.

    The identity is compared only for boxes already on the token's
    sequence, so a resolution against an unpinned sequence hashes nothing.
    """
    for box in _PINS:
        if box.seq == seq and box.sid() == sid:
            return box.items
    return None


def _process_alive(pid: int) -> bool:
    """Best-effort liveness check for the process a lease sidecar names.

    A lease whose owner process is gone expires immediately, without
    waiting out the heartbeat TTL: a killed holder can never refresh its
    heartbeat again, so its records pin nothing from this moment on. The
    check is advisory -- a pid that cannot be queried (permissions,
    platform limits, pid reuse) is treated as alive and falls back to the
    ordinary heartbeat expiry.
    """
    if pid <= 0:
        return False
    if sys.platform == "win32":
        # os.kill(pid, 0) would *terminate* the process on Windows, so
        # query its exit code through the Win32 handle API instead.
        import ctypes
    ...
    try:
        ...
        # Owned by another user, but very much alive.
        return True
    except OSError:
        return True
    return True
...
class _LeaseManager:
    ...
    A process that opens a read-only store or holds a cursor over some
    snapshot owns exactly one lease sidecar --
    ``wal.lease.<pid>.<rand>``, all of the single ``wal.lease.*`` kind --
    listing every snapshot that process has in use, one fixed-width
    ``wal.l<seq>.<sid>.<heartbeat>\\n`` record each. The file is rewritten
    atomically (temp file, rename; no fsync, because a lease need not
    survive a crash) on every change and every heartbeat, so another
    process never observes a half-written record: what it reads is one
    complete lease image or the previous one.
    ...
    timestamp is older than the TTL and sweeps the stale file. The sidecar
    name also carries the owner's pid, so a holder that has already exited
    is expired on sight -- its records pin nothing from that moment,
    without waiting out the TTL. The writer reads *all* lease sidecars
    before reclaiming a copy, so a snapshot a cursor or a read-only store
    in any process is serving is never removed.
    ...
    while the in-process pins still apply.
    """

    def __init__(self, directory: str):
        self._dir = directory
        # A unique name per process instance. The random suffix keeps two
        # incarnations of one pid (pid reuse, or processes sharing a copied
        # directory) from ever touching one sidecar.
        self._name = f"{_LEASE_PREFIX}{os.getpid()}.{uuid.uuid4().hex}"
        self._path = os.path.join(directory, self._name)
        # Snapshot keys this process currently leases, reference counted: a
        # read-only store and each of its cursors pin independently, so one
        # key stays registered until every holder has released it.
    ...
    def __del__(self) -> None:
        # Defensive backstop: if a manager is collected while it still lists
        # keys (its holders' explicit releases and finalizers normally clear
        # these first), drop the sidecar. The daemon heartbeat thread, while
        # alive, holds a reference to this manager, so by here it has exited.
        try:
            if getattr(self, "_counts", None):
                self._remove_locked()
                self._stop.set()
        except Exception:
            pass
    ...
    @staticmethod
    def _record(seq: int, sid: bytes, now: float) -> bytes:
        # Microseconds since the epoch as a fixed 16-digit integer: exactly
        # 16 digits for every realistic date (no variable-width decimal
        # point), so every record is one constant width on every platform.
        ts = str(int(now * 1_000_000)).rjust(_LEASE_TS_LEN, "0")[-_LEASE_TS_LEN:]
        return (f"{_LEASE_REC_PREFIX}{seq:0{_LEASE_SEQ_MAX}d}."
                f"{sid.hex()}.").encode("ascii") + ts.encode("ascii") + b"\n"

    def _image(self, now: float) -> bytes:
        return b"".join(self._record(seq, sid, now)
                        for seq, sid in sorted(self._counts))

    def _publish(self, now: float | None = None) -> None:
        """Atomically write this process's current lease image.

        Temp file plus a rename, deliberately *without* fsync: a lease only
        has to be visible to a concurrently running writer, never to survive
        a crash -- a crashed owner's lease is treated as stale regardless,
        and a machine crash leaves no live owner at all. Skipping the fsync
        (and the directory sync) keeps a read-only open and each heartbeat
        cheap, while the rename still guarantees the writer reads either the
        previous complete image or the new one, never a torn file.
        """
        if self._disabled:
            return
        if now is None:
        ...
        try:
            with open(tmp, "wb") as f:
                f.write(self._image(now))
            os.replace(tmp, self._path)
        except OSError:
            # A read-only directory can never host a lease: stop trying so a
            # reader there stays free of repeated failing writes. Any other
            # OSError (e.g. a brief Windows rename collision with a concurrent
            # reader of the file) is transient -- leave the lease enabled and
            # let the next heartbeat republish it, so a live holder is never
    ...
    def acquire(self, seq: int, sid: bytes) -> None:
        """Take one reference on the lease for ``(seq, sid)``.

        Idempotent across the holders of one process (a read-only store and
        each of its cursors): the key is registered once on disk but stays
        until every holder has released it.
        """
        if sid is None or seq < 1:
            # The empty initial snapshot (seq 0) never has a published copy
            # and so never needs a lease.
            return
        key = (seq, sid)
        with self._lock:
    ...
    def release(self, seq: int, sid: bytes) -> None:
        """Drop one holder's reference; remove the lease at zero. Best effort."""
        if sid is None or seq < 1:
            return
        key = (seq, sid)
        thread = None
        with self._lock:
            ...
            if self._counts:
                ...
                self._stop.set()
                self._thread = None
        # Join without holding the lock: a heartbeat already past its wait
        # may need the lock for one last publish before observing the stop.
        self._join_thread(thread)

    def close(self) -> None:
        """Retire every lease this manager's holders still hold."""
        with self._lock:
            self._remove_locked()
            self._counts.clear()
            thread = self._thread
    ...
    def _join_thread(self, thread=None) -> None:
        if thread is None:
            return
        if thread is not threading.current_thread():
            thread.join(timeout=_LEASE_HEARTBEAT * 4)
    ...
    def _remove_locked(self) -> None:
        ...
        try:
            ...
            # Left behind only if unwritable: it will expire by heartbeat.
            pass
    ...
    def _start_locked(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._run, name="wal-lease-heartbeat", daemon=True)
        self._thread.start()

    def _run(self) -> None:
        while not self._stop.wait(_LEASE_HEARTBEAT):
            with self._lock:
                if self._counts:
                    self._publish()
    ...
    @staticmethod
    def _parse_record(rec: bytes, now: float):
        """Return ``(seq, sid)`` for a fresh record, else ``None``.

        Malformed bytes, a bad sequence/identity field or a stale heartbeat
        all mean "no live user". Every sidecar is published whole, so the
        records parsed are always complete ones -- never a torn read.
        """
        if len(rec) != _LEASE_REC_LEN or not rec.endswith(b"\n"):
            return None
        try:
            text = rec.decode("ascii")
        except UnicodeDecodeError:
            return None
        if not text.startswith(_LEASE_REC_PREFIX):
            return None
        # Fixed-width layout after "wal.l": seq(20) "." sid(64) "." ts(16).
        body = text[len(_LEASE_REC_PREFIX):]
        seq_field = body[:_LEASE_SEQ_MAX]
        dot1 = body[_LEASE_SEQ_MAX:_LEASE_SEQ_MAX + 1]
        sid_field = body[_LEASE_SEQ_MAX + 1:_LEASE_SEQ_MAX + 1 + _LEASE_SID_HEX]
        dot2 = body[_LEASE_SEQ_MAX + 1 + _LEASE_SID_HEX:
                    _LEASE_SEQ_MAX + 2 + _LEASE_SID_HEX]
        ts_field = body[_LEASE_SEQ_MAX + 2 + _LEASE_SID_HEX:
                        _LEASE_SEQ_MAX + 2 + _LEASE_SID_HEX + _LEASE_TS_LEN]
        if dot1 != "." or dot2 != ".":
            return None
        try:
            seq = int(seq_field)
            sid = bytes.fromhex(sid_field)
            micros = int(ts_field)
        except ValueError:
            return None
        if len(sid) != 32 or seq < 1:
            return None
        if now - micros / 1_000_000 > _LEASE_TTL:
            return None
        return seq, sid

    @staticmethod
    def _is_lease_name(name: str) -> bool:
        """Recognise the exact ``wal.lease.<pid>.<32-hex>`` sidecar shape."""
        if not name.startswith(_LEASE_PREFIX):
            return False
        pid, sep, rand = name[len(_LEASE_PREFIX):].partition(".")
        return (bool(sep) and pid.isdigit() and len(rand) == 32
                and all(c in "0123456789abcdef" for c in rand))

    @staticmethod
    def _lease_pid(name: str) -> int:
        """The owner pid a well-named lease sidecar carries in its name."""
        pid, _sep, _rand = name[len(_LEASE_PREFIX):].partition(".")
        return int(pid)

    def _iter_lease_files(self):
        """Yield ``(name, raw)`` for each well-named lease sidecar.

        Only names of the exact lease shape are opened; anything else in the
        directory is ignored -- never parsed, never deleted.
        """
        try:
            ...
            return
        for name in names:
            if not self._is_lease_name(name):
                continue
            try:
    ...
    def _iter_live_lease_files(self):
        """Yield ``(name, raw)`` for sidecars whose owner process is alive.

        A sidecar whose named pid no longer runs belongs to a process that
        ended without closing: its records are stale no matter how fresh
        the heartbeat looks, so it is neither consulted nor yielded here.
        """
        for name, raw in self._iter_lease_files():
            if _process_alive(self._lease_pid(name)):
                yield name, raw

    def live_keys(self, now: float | None = None) -> set:
        """The ``(seq, sid)`` pairs with a fresh lease in any process.

        Every fresh record in every well-named lease sidecar names an in-use
        snapshot; stale heartbeats, a dead owner process (a killed holder)
        and malformed records are ignored. Reading never mutates the
        registry.
        """
        if now is None:
            now = time.time()
        live: set[tuple[int, bytes]] = set()
        for _name, raw in self._iter_live_lease_files():
    ...
    def prune(self, now: float | None = None) -> None:
        ...
        process is gone, belongs to a process that ended without closing
        (its heartbeat stopped) and is swept; a sidecar with at least one
        fresh record from a live process is left untouched. Each unlink is
        independent, so a kill mid-sweep simply finishes on the next
        reclaim. This process's own sidecar is never pruned while it holds
        keys.
        """
        if now is None:
            now = time.time()
...
# on one directory in this process (and every cursor, on either open form)
# share a manager and therefore one lease sidecar, so independently closing
# stores and cursors reference-count the same on-disk records. A manager
# with no holders keeps neither a thread nor a file; reusing it later
# merely repopulates one. The map is weak on its values -- every open store
# on the directory holds a strong reference, so a manager is collected once
# they are all gone and does not accumulate across a long process -- and a
# writer borrows the same keyed manager to scan and prune the cross-process
# leases.
...
def _close_all_leases() -> None:
    """Interpreter-exit backstop: remove every lease this process held.

    A reader that exits normally without an explicit ``close()`` (a common
    short script) reaches here rather than a SIGKILL, so its sidecar is
    cleaned up instead of waiting out the TTL. Hard kills still expire by
    heartbeat.
    """
    try:
        managers = list(_LEASES.values())
    except Exception:
        return
...
def _lease_manager(directory: str) -> _LeaseManager:
    global _LEASE_ATEXIT
    key = os.path.abspath(directory)
    manager = _LEASES.get(key)
    if manager is None:
        manager = _LeaseManager(key)
        _LEASES[key] = manager
        if not _LEASE_ATEXIT:
            atexit.register(_close_all_leases)
            _LEASE_ATEXIT = True
    return manager
...
class Store:
    ...
    With ``read_only=True`` the store is opened purely for reading. No log,
    checkpoint or snapshot file is ever created or modified, and no lock is
    taken: any number of read-only processes may coexist with the one
    writer. The one file a reader does create is its own short-lived lease
    sidecar registering its pinned snapshots cross-process (best-effort; a
    read-only directory simply has none). The reader pins the committed
    snapshot current at open time -- the highest committed prefix of
    ``wal.log`` or of the atomically replaced ``wal.ckp`` sidecar -- scans
    it once, and serves every ``get`` from that in-memory index. The
    ...
    def __init__(self, path: str, read_only: bool = False):
        ...
        self._closed = False
        self._corrupt = False
        # Read-only stores pin their open-time snapshot in the process-wide
        ...
        # store on it in this process. A read-only store acquires a lease
        # for its open-time snapshot, and every cursor and resume session --
        # on either open form -- acquires one for the snapshot it pins; the
        # writer store itself never registers one (its current snapshot is
        # kept by the retention rule) and otherwise only reads and prunes
        # the registry.
        ...
        # Lazily computed content identity of the writer's committed
        # snapshot, cached as ``(seq, sid)`` so a cursor opening on an
        # unchanged committed state does not re-hash it. The committed
        # state at one sequence number is immutable, so the sequence alone
        # keys the cache.
        self._sid_cache: tuple[int, bytes] | None = None
    ...
    def _remove_stale_temps(self) -> None:
        ...
        # by a kill is stale by seconds and can never be renamed again.
        now = time.time()
        for name in os.listdir(self._dir):
            if not (name.startswith(_LEASE_PREFIX)
                    and name.endswith(".tmp")):
    ...
    def _open_read_only(self) -> None:
        """Pin a committed snapshot and register its cross-process lease.

        No lock is taken and the log, checkpoint and snapshot copies are
        never touched; the one file written is this reader's short-lived
        lease sidecar (best-effort). The reader chooses the highest
        committed prefix available at the instant it opens -- the validated
        prefix of ``wal.log`` or the existing ``wal.ckp`` sidecar, which the
        writer replaces atomically as part of every commit -- scans that
        prefix once into memory, and serves every ``get`` from that index.
        Later reads are independent of log history, never replay the log
        ...
        self._ro_sid = _snapshot_id(items)
        # A live reader counts as a user of the snapshot it pinned: its box
        # stays in the process-wide pin set until the reader closes, so the
        # writer's reclamation never removes that snapshot's copy.
        self._pin_box = _register_snapshot(seq, items, self._ro_sid)
        # Make the same pin visible to a writer in another process: register
        # a short-lived cross-process lease for the open-time snapshot. The
        # write is best-effort, so a read-only directory simply skips it.
        self._lease_key = (seq, self._ro_sid)
        self._leases.acquire(seq, self._ro_sid)
        # A reader dropped without close() must release its lease the way its
        # in-process pin releases when it is collected.
        self._lease_finalizer = weakref.finalize(
            self, _release_lease, self._leases, seq, self._ro_sid)
    ...
    def scan(self, start: str | None = None, end: str | None = None,
        ...
        # whichever open form it is on: it leases its pinned snapshot (the
        # reader's open-time one here, so the identity is already known;
        # the writer's committed one, cached per sequence) and a writer in
        # any process then keeps that copy -- even after the store this
        # cursor came from has closed.
        if self._read_only:
            sid = self._ro_sid
        else:
            sid = self._committed_snapshot_id(seq, items)
        return ScanCursor(
            items, lo_idx, hi_idx, seq=seq, lo=lo, hi=hi,
            publisher=self._snapshot_publisher(),
            leases=self._leases, sid=sid)

    def _committed_snapshot_id(self, seq: int, items) -> bytes | None:
        """Content identity of the writer's committed snapshot at ``seq``.

        Cached per sequence number: the committed state at one sequence is
        immutable, so repeated scans between commits hash it only once.
        The empty initial snapshot (sequence 0) has no published copy and
        needs no lease, so it carries no identity here.
        """
        if seq < 1:
            return None
        cache = self._sid_cache
        if cache is None or cache[0] != seq:
            cache = (seq, _snapshot_id(items))
    ...
    def _publish_snapshot(self, seq: int, sid: bytes | None,
        ...
        reclaims. An existing file with the same name is trusted only after
        its content verifies against the name's identity; a corrupt one is
        no copy at all and is atomically replaced with the authoritative
        image. Writer-only: read-only stores never publish snapshot copies
        (their cross-process pin rides the lease sidecar instead). The
        ...
        if (os.path.exists(path)
                and self._load_snapshot_copy(seq, sid) is not None):
            return
    ...
    def _parse_snapshot_name(self, name: str):
        ...
        try:
            seq = int(stem[:dot])
            sid = bytes.fromhex(stem[dot + 1:])
        except ValueError:
            return None
        if seq < 1 or len(sid) != 32:
            return None
        return seq, sid

    def _in_use_keys(self) -> set:
        """Snapshots in use anywhere: this process plus every lease.

        The union of the process-wide weak pin set (open cursors and live
    ...
    def _reclaim_snapshots(self) -> None:
        ...
        Every candidate's content is verified against the identity its
        name carries before it counts as a copy at all: a file whose
        content does not match is corrupt -- unusable to every reader --
        and is removed here like any other dead copy, never served and
        never repaired. Corrupt files do not occupy a retention slot.
        ...
        or its pid gone) are swept first. The scan is idempotent and
        kill-safe: each unlink is independent, re-running reaches the same
        set of files, and a run killed at any unlink is simply finished on
        the next open; malformed sidecar names are ignored rather than
        treated as files to delete.
        """
        copies = []   # usable copies: (seq, sid, name)
        corrupt = []  # well-named files whose content fails verification
        try:
            names = os.listdir(self._dir)
        except FileNotFoundError:
            return
        for name in names:
            ...
            if parsed is None:
                continue
            seq, sid = parsed
            if self._load_snapshot_copy(seq, sid) is not None:
        ...
        self._leases.prune()
        removed = False
        # Corrupt copies are dead weight: no reader can ever use them, so
        # no pin, lease or retention slot protects them.
        for name in corrupt:
            try:
                os.unlink(os.path.join(self._dir, name))
            except FileNotFoundError:
    ...
    def _load_snapshot_copy(self, seq: int, sid: bytes):
        """Load a published snapshot sidecar if its content proves its name.

        Returns the sorted items when ``wal.s<seq>.<id>`` exists and
        verifies -- a clean frame sequence of one snapshot header carrying
        the name's sequence followed by put frames with strictly increasing
        keys whose canonical image hashes to the name's content identity.
        Returns ``None`` when the file is absent, unreadable or fails any
        check: a copy whose content does not match its content-addressed
        name is corrupt, and corrupt copies are never served, never guessed
        at and never repaired. The caller falls back to the committed log
        prefix or the checkpoint, and the reclamation sweep removes the
        file.
        """
        path = self._snapshot_blob_path(seq, sid)
        try:
            ...
            # Absent, or unreadable for any reason: not a usable copy.
            return None
    ...
    def _resolve_token_snapshot(self, seq: int, sid: bytes) -> list:
        ...
        the log or checkpoint. A sidecar whose content fails its identity
        check is corrupt and counts as no copy at all -- the rebuild
        sources are tried instead, and only when they also fail is the
        token rejected. The empty initial snapshot (sequence 0) is always
        reconstructible. Anything else is a forged, stale or foreign
        ...
        if cur_seq == seq:
            ...
            if _snapshot_id(items) == sid:
                return items
        items = self._load_snapshot_copy(seq, sid)
        if items is not None:
            return items
    ...
    def _resume_scan(self, tok) -> ScanCursor:
        ...
        if not isinstance(tok, bytes):
            # Only genuine bytes are accepted; a convertible stand-in
            # (bytearray, memoryview, str, ...) is rejected rather than
            # coerced.
        ...
        # keeps its copy; the in-process pin covers the same store's own
        # reclamation.
    ...
    def close(self) -> None:
        if self._closed:
            return
        if self._read_only:
            if self._pin_box is not None:
                _unpin_snapshot(self._pin_box)
                self._pin_box = None
            # Release just this reader's cross-process reference; a cursor
            # still open keeps its own reference (and the sidecar) alive.
            finalizer = getattr(self, "_lease_finalizer", None)
            if finalizer is not None:
                finalizer.detach()
            if self._lease_key is not None:
                self._leases.release(*self._lease_key)
                self._lease_key = None
